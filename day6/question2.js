...
function search(instructions){
  ...
  instructions.forEach(function(instruction){
    var command = parseInstructionCommand(instruction);
    for(var x=command.start.x;x<=command.end.x;x++){
      if(!grid[x]) grid[x] = [];
      for(var y=command.start.y;y<=command.end.y;y++){
        grid[x][y] = command.action(grid[x][y]);
      }
    }
  });
  var brightness = countLightsBrightness(grid);
  return brightness;
}

function parseInstructionCommand(instruction){
  var command = {};
  if(instruction.indexOf('turn on')==0){
    command.action = function(val){
      return val ? val+1 : 1;
    };
  }else if(instruction.indexOf('turn off')==0){
    command.action = function(val){
      return val ? Math.max(val-1,0) : 0;
    };
  ...
  var coords    = instruction.match(/[0-9]+,[0-9]+/g);
  command.start = parseCoords(coords[0]);
  command.end   = parseCoords(coords[1]);
  return command;
}
...
// The Solution
// ------------------------------------------------
// console.log(search(['turn on 0,0 through 1,1']), 'lights are lit!');
console.log('The grid has a brightness of', search(INPUT.split(';')));