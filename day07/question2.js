...
function search(instructions, onSet){
  ...
  function setValue(target, value){
    // If we have a onSet callback we use it now to allow some override
    if(typeof onSet == 'function'){
      value = onSet(target, value);
    }
    // Simulate the sign change
    if(value<0){
  ...
  // We'll execute the commands as long as all the instructions are not done
  while(commands.length>0){
    // execute the commands
    var command = commands.shift(); //FIFO
    // Read the values (if they exist)
    var a = getValue(command.a);
    var b = getValue(command.b);
    ...
    switch(command.operator){
      case 'SET':
        if(a!==false){
          setValue(command.target, a);
          success = true;
        }
        break;
      case 'NOT':
        // If we know the value of the negated wire
        if(a!==false){
          setValue(command.target, ~ a);
          success = true;
        }
        break;
      case 'AND':
        // If we know the value of the both wires
        if(a!==false && b!==false){
          setValue(command.target, a & b);
          success = true;
        }
        break;
      case 'OR':
      ...
      case 'RSHIFT':
        ...
        break;
      default:
        console.error('DEFAULT'); // Should not happen
    }
    if(!success){
...
function parseInstruction(instruction){
  var command = {};
  var affectation = instruction.split(' -> ');
  command.target  = affectation[1];
  var operation   = affectation[0].split(' ');
  
  if(operation.length==1){
    command.operator = 'SET';
    command.a = operation[0];
  
  }else if(operation.length==2){
    command.operator = operation[0]; // NOT
    command.a        = operation[1]; // Wire
  
  }else if(operation.length==3){
    command.operator  = operation[1];
    command.a         = operation[0];
    command.b         = operation[2];
  } 
  command.source = instruction; // DEBUG
  return command;
}
...
// We run the instructions once
var wires = search(instructions);
// Then we run it again but forcing it to override B with A
wires = search(instructions, function(target, value){
  return target=='b' ? wires.a : value;
});

console.log('The value of the wire A is:', wires.a);